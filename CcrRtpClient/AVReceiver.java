	...
	ControllerListener, RemoteListener
{
    ...
    Vector targets;
    CcrRtpClient newMainCcr;
    DataSource ds;
    
    public AVReceiver( CcrRtpClient newMainCcr, Vector targets) {
	this.newMainCcr= newMainCcr;
    ...
    public void addTarget( String senderAddress,
        ...
        try {
	    InetAddress ipAddr;
	    SessionAddress localAddr;//rtp addresses
	    ...
	    ipAddr = InetAddress.getByName( senderAddress);//creates an internet address for the sender
	    ...
	    mgr.initialize( localAddr);// an rtpmanager method that says the rtp session is set up
	    ...
	    if (bc != null) {
    	    ...
    	    mgr.addTarget(destAddr);//This method opens the session, causing RTCP reports to be
 									//generated and callbacks to be made through the
 									//SessionListener interface. This method must be called after
 									//session initialization and prior to the creation of any streams on a session.

	    mgrs.addElement( mgr);// add the rtpmanagers to the vector mgrs	    
        } catch (Exception e){
	...
	PlayerWindow(Player p, ReceiveStream strm) {
	   ...
	   setTitle( "Cork Campus Radio RTP Player");
	   addWindowListener(
         new WindowAdapter() {
